...
struct MoroGamesGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://morogamesx.top/get")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            MoroGamesEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    MoroGamesGameInitialView()
}
