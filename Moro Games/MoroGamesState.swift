...
protocol MoroGamesLoadStateRepresentable {
    var type: MoroGamesLoadState.StateType { get }
...
struct MoroGamesLoadState: Equatable, MoroGamesLoadStateRepresentable {
    ...
    static func idle() -> MoroGamesLoadState {
        MoroGamesLoadState(type: .idle, percent: nil, error: nil)
    }

    /// Создание состояния прогресса
    static func progress(_ percent: Double) -> MoroGamesLoadState {
        MoroGamesLoadState(type: .progress, percent: percent, error: nil)
    }

    /// Создание состояния успеха
    static func success() -> MoroGamesLoadState {
        MoroGamesLoadState(type: .success, percent: nil, error: nil)
    }

    /// Создание состояния ошибки
    static func error(_ err: String) -> MoroGamesLoadState {
        MoroGamesLoadState(type: .error, percent: nil, error: err)
    }

    /// Создание состояния отсутствия подключения
    static func offline() -> MoroGamesLoadState {
        MoroGamesLoadState(type: .offline, percent: nil, error: nil)
    }

    // MARK: - Методы сравнения

    /// Пользовательская реализация сравнения
    func isEqual(to other: MoroGamesLoadState) -> Bool {
    ...
    static func == (lhs: MoroGamesLoadState, rhs: MoroGamesLoadState) -> Bool {
        lhs.isEqual(to: rhs)
    }
}
...
extension MoroGamesLoadState: CustomStringConvertible {
